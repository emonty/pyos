...
USER_AGENT = "pyos/%s" % version.version
...
class Settings(object):
    ...
    environment = property(_getEnvironment, _setEnvironment, None,
            """Users can define several environments for use with pyos. This
            ...
            services, such as `pyos.cloudservers`, to None. You must
    ...
    def read_config(self, config_file):
        ...
        for section in cfg.sections():
            ...
            if not self._default_set:
                ...
                self._default_set = True
        if creds_found:
            warnings.warn("Login credentials were detected in your .pyos.cfg "
...
def clear_credentials():
    ...
    global cloud_networks, cloud_monitoring, images, queues
    identity = None
    regions = tuple()
    services = tuple()
    cloudservers = None
...
def _make_agent_name(base):
    """Appends pyos information to the underlying library's user agent."""
    if base:
        if "pyos" in base:
            return base
        else:
...
settings = Settings()
config_file = os.path.join(os.path.expanduser("~"), ".pyos.cfg")
...
# Set up logging
_logger = logging.getLogger("pyos")
