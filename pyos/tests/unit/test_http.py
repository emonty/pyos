...
class HttpTest(unittest.TestCase):
    ...
    def test_http_log_req(self):
        ...
        uri = utils.random_unicode()
        sav_pdbug = pyos._http_debug
        pyos._http_debug = False
        self.assertIsNone(self.http.http_log_req(mthd, uri, args, kwargs))
        pyos._http_debug = True
        sav_pldbug = pyos._logger.debug
        pyos._logger.debug = Mock()
        self.http.http_log_req(mthd, uri, args, kwargs)
        pyos._logger.debug.assert_called_once_with(
                "\nREQ: curl -i -X %s a b -H 'c: C' %s\n" % (mthd, uri))
        kwargs["body"] = "text"
        self.http.http_log_req(mthd, uri, args, kwargs)
        cargs, ckw = pyos._logger.debug.call_args
        self.assertEqual(cargs, ("REQ BODY: text\n", ))
        pyos._logger.debug = sav_pldbug
        pyos._http_debug = sav_pdbug

    def test_http_log_resp(self):
        log = logging.getLogger("pyos")
