...
def http_log_req(method, uri, args, kwargs):
    """
    When pyos.get_http_debug() is True, outputs the equivalent `curl`
    command for the API request being made.
    """
    if not pyos.get_http_debug():
        return
    ...
    log.debug("\nREQ: %s\n" % " ".join(string_parts))
    if "body" in kwargs:
        pyos._logger.debug("REQ BODY: %s\n" % (kwargs["body"]))
    if "data" in kwargs:
        pyos._logger.debug("REQ DATA: %s\n" % (kwargs["data"]))


def http_log_resp(resp, body):
    """
    When pyos.get_http_debug() is True, outputs the response received
